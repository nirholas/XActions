# XActions — Build Prompts v2: What People Actually Want

**Philosophy:** Don't reinvent the wheel. Clone the best OSS, integrate into XActions, innovate on top.

Each prompt tells the AI: what exists, what to clone, what's already built in XActions, and what to add.

---

## Quick Reference: What's Already Built vs What Needs Work

| Feature | XActions Has | Status | Action Needed |
|---------|-------------|--------|---------------|
| Video Downloader | `src/scrapers/videoDownloader.js` | Real (browser console) | Need **web UI** at xactions.app/video |
| Thread Unroller | `src/scrapers/threadUnroller.js` | Real (browser console) | Need **web UI** at xactions.app/thread |
| Unfollower Detector | `src/detectUnfollowers.js` | Real (browser console) | Need **dashboard page** with daily tracking |
| Sentiment Analysis | `src/analytics/sentiment.js` | Real (391 lines, AFINN lexicon) | Need **dashboard page** |
| Best Time to Post | `src/bestTimeToPost.js` | Real (browser console) | Need **dashboard page** |
| Competitor Analysis | `src/competitorAnalysis.js` | Real (browser console) | Need **dashboard page** |
| Account Export | `src/portability/exporter.js` | Real (Node.js) | Need **CLI polish + web UI** |
| Bluesky Scraper | `src/scrapers/bluesky/index.js` | Real (AT Protocol) | Needs testing |
| Plugin System | `src/plugins/loader.js` | Real | Low priority |
| Browser Extension | Nothing | **NOT BUILT** | Build from scratch |
| AI Tweet Writer | Nothing | **NOT BUILT** | Build from scratch |
| Web Tools (video/thread) | Nothing | **NOT BUILT** | Build web UIs |

---

## Repos to Clone & Learn From (Don't Reinvent)

| Repo | Stars | License | What to Take |
|------|-------|---------|-------------|
| [insin/control-panel-for-twitter](https://github.com/insin/control-panel-for-twitter) | 2,460 | MIT | Chrome extension architecture, Manifest V3 patterns, x.com content scripts |
| [nicedoc/thread-reader](https://github.com/nicedoc/thread-reader) | — | MIT | Thread unrolling web UI patterns |
| [sindresorhus/refined-twitter](https://github.com/sindresorhus/refined-twitter) | 1,306 | MIT | Chrome extension for Twitter, clean Manifest V3 structure |
| [imputnet/cobalt](https://github.com/imputnet/cobalt) | 38,896 | AGPL-3.0 | Video extraction API patterns (⚠️ AGPL — study, don't copy code) |
| [imtonyjaa/twitterxdownload](https://github.com/imtonyjaa/twitterxdownload) | 379 | AGPL-3.0 | Twitter video download web app (⚠️ AGPL — study, don't copy code) |
| [saifalfalah/tvdl-2](https://github.com/saifalfalah/tvdl-2) | 118 | Apache-2.0 | Twitter video download approach (✅ can adapt) |

---

## Prompt 1: Browser Extension (THE #1 Priority)

```
I'm building a Chrome/Firefox extension for XActions (github.com/nirholas/XActions), an open-source X/Twitter automation toolkit.

## What Already Exists in XActions

XActions already has 20+ working browser console scripts in src/automation/ that users paste into DevTools. They work, but require technical knowledge. Key scripts:

- src/automation/core.js — foundation with SELECTORS object (data-testid based), CONFIG (delays/limits), helpers (sleep, scrollToBottom, waitForElement, clickElement)
- src/automation/autoLiker.js — auto-likes by keyword
- src/automation/smartUnfollow.js — unfollows non-followers
- src/automation/autoCommenter.js — auto-comments
- src/automation/followEngagers.js — follows people who engage with tweets
- src/automation/keywordFollow.js — follows users by keyword
- src/automation/growthSuite.js — combined growth automation
- src/scrapers/videoDownloader.js — downloads videos from tweets
- src/detectUnfollowers.js — detects who unfollowed you
- src/bestTimeToPost.js — analyzes best posting times

## OSS to Study (Don't Copy AGPL Code)

Clone these MIT-licensed repos to learn their architecture:

1. **insin/control-panel-for-twitter** (2,460 stars, MIT) — THE reference. Study their:
   - Manifest V3 structure
   - Content script injection on x.com
   - Settings management via chrome.storage
   - How they modify Twitter's UI
   - Their approach to selectors

2. **sindresorhus/refined-twitter** (1,306 stars, MIT) — Study their:
   - Clean extension architecture
   - Feature toggle patterns
   - x.com DOM manipulation

Run: `git clone https://github.com/insin/control-panel-for-twitter.git /tmp/cpft && ls -la /tmp/cpft/`

## What to Build

Create `extension/` directory with a Chrome Manifest V3 extension:

### Architecture
- extension/manifest.json — Manifest V3, host_permissions for x.com and twitter.com only
- extension/popup/ — popup.html, popup.css, popup.js
- extension/content/ — content.js (injected into x.com pages)
- extension/background/ — service-worker.js
- extension/icons/ — 16, 48, 128px icons with "XA" branding
- extension/README.md — install instructions

### Popup UI (7 Feature Cards)
Dark theme (#000 bg, #1d9bf0 accent, #e7e9ea text) matching X's aesthetic.

Each card has: name, icon emoji, description, ON/OFF toggle, settings gear icon.

1. **Unfollow Non-Followers** — Scans following list, unfollows anyone not following back
   - Settings: delay between actions (1-5s), skip verified, skip accounts older than X days
2. **Auto-Liker** — Likes tweets matching keywords in your feed/search
   - Settings: keywords list, max likes/session, delay
3. **Video Downloader** — Adds download button to every tweet with video
   - Settings: auto-download on/off, quality preference
4. **Who Unfollowed Me** — Shows recent unfollowers
   - Settings: check frequency, notification on/off
5. **Best Time to Post** — Analyzes your engagement patterns
   - Shows heatmap of best times
6. **Thread Reader** — Unrolls threads into clean readable view
   - Adds "Unroll" button to threads
7. **Quick Stats** — Shows engagement rate on your profile
   - Always-on dashboard overlay

### Content Script Strategy
- Content script injects a `<script>` tag into page context (needed for window.__NEXT_DATA__ access)
- Communication: content.js ↔ popup.js via chrome.runtime.sendMessage
- Detection: content script watches URL changes to know when user navigates
- Core automation functions bundled from src/automation/core.js patterns (rewrite, don't import — extension must be standalone)

### Key UX Details
- Extension badge shows action count (number of likes, unfollows, etc.)
- Right-click context menu: "Download video", "Unroll thread", "Analyze account"
- First-run onboarding: explain what extension does, one-click enable popular features
- Rate limit detection: automatically pause and show warning
...
Vanilla JS only, no framework. Must work on both x.com and twitter.com domains.
...
## Prompt 2: Video Downloader Web Tool (Traffic Magnet)

```
I'm building a web-based Twitter/X video downloader page for XActions (github.com/nirholas/XActions).

## What Already Exists

XActions has a working video downloader in src/scrapers/videoDownloader.js (361 lines). It's a browser console script that:
- Extracts video URLs from React's __NEXT_DATA__ state
- Falls back to DOM video element extraction
- Intercepts network requests for .mp4 URLs
- Offers quality selection (highest bitrate auto-selected)
- Auto-triggers download

The project also has an Express.js backend at api/server.js and a dashboard at dashboard/.

## OSS to Study

- **imputnet/cobalt** (38.9k stars, AGPL) — Study their API design and UX, NOT their code (AGPL is viral). Their UX: paste URL → get video. That's it.
- **saifalfalah/tvdl-2** (118 stars, Apache-2.0) — Study their Twitter video extraction approach. Can adapt patterns.

Run: `git clone https://github.com/saifalfalah/tvdl-2.git /tmp/tvdl && cat /tmp/tvdl/package.json`

Key learning: Twitter embeds video URLs in DOM/API responses. The best approach for server-side is to use Puppeteer to load the tweet page and extract the video URL from the page's internal state, then redirect the user to the direct MP4 URL.

## What to Build

### 1. Dashboard Page: dashboard/video.html

Minimal, fast, beautiful. Dark theme matching existing dashboard.

UI:
- Big input field: "Paste tweet URL here"
- Big blue button: "Download"
- Loading spinner during extraction
- Result: video thumbnail preview + download button with quality options (1080p, 720p, 480p)
- Error states: invalid URL, no video found, rate limited
- Recent downloads history (localStorage)
- Share button: "Get XActions — free video downloader"

No login required. No signup. Just paste and download.

### 2. API Route: api/routes/video.js

POST /api/video/extract
- Input: { url: "https://x.com/user/status/123" }
- Validation: must be x.com or twitter.com URL with status ID
- Process: use Puppeteer (from existing scraper infrastructure) to load tweet → extract video URLs
- Response: { videos: [{ url, quality, width, height, bitrate, contentType }], thumbnail, duration, author, text }
- Rate limit: 30 requests/minute per IP (use existing express-rate-limit)
- Cache: Redis cache by tweet ID (videos don't change), TTL 1 hour

GET /api/video/download?url=<encoded_mp4_url>
- Proxies the video download (avoids CORS issues)
- Streams response (don't buffer entire video in memory)
- Sets Content-Disposition header for filename: {author}_{tweetId}.mp4

### 3. Puppeteer Extraction (api/services/videoExtractor.js)

Use the Browser Pool from src/streaming/browserPool.js if it exists, or create a simple one:
- Pool of 2 Puppeteer browsers max
- Load tweet URL with stealth plugin
- Wait for video element or __NEXT_DATA__ to contain video info
- Extract all video variants (different qualities)
- Close page, return to pool
- Timeout: 15 seconds max per extraction
- Fallback: try Twitter's public embed endpoint (publish.twitter.com)

### 4. SEO

The page should have meta tags optimized for "twitter video downloader", "download x video", "save twitter video":
- Title: "Twitter Video Downloader — Free, No Login | XActions"
- Description: "Download videos from Twitter/X for free. No signup, no app install. Just paste the tweet URL."
- Open Graph tags for social sharing

### 5. Add to Navigation

Add "Video Downloader" link to the dashboard nav bar and footer on all existing dashboard pages.

Make it work without JavaScript for the basic form (progressive enhancement) — form POST that redirects to download. JS enhances with async extraction + preview.
```
