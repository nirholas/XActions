...
router.post('/sync/:username', async (req, res) => {
  try {
    ...
    const result = await syncFollowers(req.user.id, req.params.username);
    res.json(result);
  } catch (error) {
...
router.post('/tag', async (req, res) => {
  try {
    ...
    tagContact(req.user.id, username, tag);
...
router.get('/search', async (req, res) => {
  try {
    ...
    const results = searchContacts(req.user.id, req.query.q || '');
...
router.get('/segment/:name', async (req, res) => {
  try {
    ...
    const members = getSegment(req.user.id, req.params.name);
...
router.post('/score', async (req, res) => {
  try {
    ...
    const result = autoScore(req.user.id);
    res.json(result);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
