...
Each test has 30s timeout. Log rate limit status after each. All fixtures are real API responses, not mocked.
```
...
- Global test setup (e.g., suppress console.warn in tests unless XACTIONS_DEBUG)
- Register custom matchers if needed
...
2. Test directory structure and what each directory tests
3. How to write new tests (import helpers, load fixtures, mock HTTP)
4. How to add fixtures (capture real responses, anonymize, save as JSON)
5. Integration test setup (env vars, cookies, expected behavior)
6. CI pipeline description
7. Coverage goals: client/ 90%, scrapers/ 70%, mcp/ 80%, cli/ 75%
8. Troubleshooting: flaky tests, rate limits during integration, fixture updates
9. Performance benchmark instructions
